...
for com in compounds:
    ...
    for nr in range(NREP):
        ...
        for f in files:
            ...
            eq_dHdl = ss.equilibrium_detection(dHdl, series=srs)
            eq_u_nkr = ss.equilibrium_detection(u_nkr, series=srs)
            eq_srs = ss.equilibrium_detection(srs, series=srs)
            list_data_TI.append(eq_dHdl)
            list_data_BAR.append(eq_u_nkr)
...
for com in compounds:
    ...
    for nr in range(NREP):
        ...
        for f in files:
            ...
            eq_u_nkr = ss.equilibrium_detection(u_nkr, series=srs)
            eq_srs = ss.equilibrium_detection(srs, series=srs)
            list_data_UC_TI.append(ss.statistical_inefficiency(eq_dHdl, series=eq_srs))
            list_data_UC_BAR.append(ss.statistical_inefficiency(eq_u_nkr, series=eq_srs))
        
        #for TI estimator
        dhdl = pd.concat([ld for ld in list_data_UC_TI])
        ti = TI().fit(dhdl)
        sum_ti = ti.delta_f_.loc[[(0.0, 0.0)], [(1.0, 1.0)]].values[0][0] * k_b_T
        sum_ds_ti = ti.d_delta_f_.loc[[(0.0, 0.0)], [(1.0, 1.0)]].values[0][0] * k_b_T
        tis.append(sum_ti)
        ...
        #for MBAR estimator
        u_nk = pd.concat([ld for ld in list_data_UC_BAR])
        mbar = MBAR().fit(u_nk)
        sum_mbar = mbar.delta_f_.loc[[(0.0, 0.0)], [(1.0, 1.0)]].values[0][0] * k_b_T
        sum_ds_mbar = mbar.d_delta_f_.loc[[(0.0, 0.0)], [(1.0, 1.0)]].values[0][0] * k_b_T
        mbars.append(sum_mbar)
